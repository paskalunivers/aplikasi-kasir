...
if (isset($_POST['import'])) {
  ...
  if ($Ipembayaran < $itungtrans3) {
      echo '<script>alert("Pembayaran tidak mencukupi!");history.go(-1);</script>';
      exit(); // Menghentikan proses jika pembayaran tidak cukup
  }

  $Ikembalian = htmlspecialchars($_POST['kembalian']);
  
  // Proses penyimpanan data setelah validasi
  $UpdCart = mysqli_query($conn, "UPDATE inv SET pembayaran='$Ipembayaran', kembalian='$Ikembalian', status='selesai' WHERE invoice='$noinv'");
  $UpdLap = mysqli_query($conn, "INSERT INTO laporan (invoice,kode_produk,nama_produk,harga,harga_modal,qty,subtotal) SELECT invoice,kode_produk,nama_produk,harga,harga_modal,qty,subtotal FROM cart");
  $DelCart = mysqli_query($conn, "DELETE FROM cart");

  if ($UpdCart && $UpdLap && $DelCart) {
      echo '<script>window.location="invoice.php?detail=' . $noinv . '"</script>';
  } else {
      echo '<script>alert("Gagal Disimpan");history.go(-1);</script>';
  }
}
...
<!-- end Modal Exit -->
    ...
    <script type="text/javascript">
      //Mengubah nilai input berdasarkan produk yang dipilih
      ...
      // Fungsi untuk menghitung kembalian dan validasi pembayaran
      function procesBayar() {
          ...
          // Validasi jika pembayaran kurang
          if (pembayaran_Cart < harga_Cart) {
              alert("Pembayaran tidak mencukupi!");
              document.getElementById('pembayaran').value = "0";  // Reset nilai pembayaran
              document.getElementById('kembalian').value = "0";  // Reset nilai kembalian
              document.getElementById('kembalian1').value = "0"; // Reset nilai hidden kembalian
          ...
          var number_string = kembali_Cart.toString(),
              ...
              //\d{3}: Mencari grup yang terdiri dari tepat tiga digit angka.
              //g: Global flag, mencari semua grup tiga digit dalam string.
              //i: Tidak sensitif huruf, tetapi di sini tidak relevan karena hanya angka yang diproses
